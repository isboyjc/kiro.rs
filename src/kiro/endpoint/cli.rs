...
// 阶段 2 移植：以下 runtime API user-agent 辅助函数及 CLI_RUNTIME_API_VERSION
// 常量目前仅在 usage_request_parts 路径使用，而该 trait 方法的 caller 要到
// 阶段 4 重构 token_manager 时才接入。届时本 allow 可移除。
#![allow(dead_code)]
